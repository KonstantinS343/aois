...
class Numbers():
    ...
    def translate_int_to_binary_direct_code(self, number: int,  not_sign: bool = False) -> List[int]:
        ...
        while temp != 0 and temp != 1:
            remains_from_divisions = temp % 2
            temp //= 2
            binary_number.append(remains_from_divisions)
        binary_number.append(temp)

        return insert_zero_for_byte(self, binary_number, number, not_sign)
    ...
    def product_numbers(self, temp_first_number: List[int], temp_second_number: List[int]) -> int:
        ...
        for i in range(len(temp_first_number) - 2, -1, -1):
            for j in range(len(temp_second_number) - 1, -1, -1):
                temp[j] = temp_first_number[i] * temp_second_number[j]

            output = self.addition_of_numbers(
                list(reversed([0] + output)),
                list(reversed(temp + counter * [0])))
            counter += 1
        output[0] = sign
    ...
    def division_numbers(self) -> int:
        ...
        def align_size(first_number: List[int], second_number: List[int]):
            if not more_than(first_number, second_number):
        ...
        def more_than(first_number: List[int], second_number: List[int]) -> bool:
            if len(first_number) > len(second_number):
                return True
        ...
        def check_zero(first_number: List[int], second_number: List[int]):
        ...
        temp_second_number = [0] + \
            self.__second_number_in_dicrect_code.copy()[1:]
        temp_fist_number, temp_second_number = align_size(
            temp_fist_number, temp_second_number)
        ...
        check_zero(temp_fist_number, temp_second_number)
        while more_than(temp_fist_number, positive_second_number):
    ...
    def translate_float_to_binary(self, float_number: str) -> List[List[int]]:
        def delete_zero(number: List[int]) -> List[int]:
        ...
        int_part_in_binary = delete_zero(int_part_in_binary)
        if int_part_in_binary != [0]:
            ...
            index, float_part_in_binary = normilaze(float_part_in_binary)   
    ...
    def addition_floating_number(self) -> float:
        def move(number: List[object], difference):
            number.remove(',')
            number = [0]*difference + number
            number.insert(1,',')
            return number
        
        def align_exponents() -> Tuple[List[int],List[int],int]:
    ...
    def translate_float_part_of_number_in_direct_code(self, float_number: List[int], mantiss_size: int) -> List[int]:
        ...
        result = []
        while len(result)<number_precision and temp != 1.0:
            temp = temp*2
            result.append(int(str(temp)[0]))
            if temp>1:
                temp -= 1
        return result